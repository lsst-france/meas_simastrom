...
bool MatchExposure(ExposureCatalog &EC, const Point &TangentPoint, const SimAstromControl &AstromControl)
{
  ...
  match = ListMatchCollect((BaseStarList&)tpCat, refCat, &guessCorr, AstromControl.linMatchCut/3600);
  ...
  if (match->size() < AstromControl.linMatchMinCount)
    ...
    throw LSST_EXCEPT(lsst::pex::exceptions::RuntimeError,"ERROR: not enough matches : giving up (if you think the match is OK, alter AstromControl.linMatchMinCount");
    }
  ...
  for (unsigned k=0; k<chips.size(); ++k)
      ...
      for (StarMatchCIterator i = match->begin(); i!= match->end(); ++i)
	{
      ...
      if (subList.size() < AstromControl.minMatchPerChip)
	{
	  std::cout << "ERROR: not enough matches for current chip: giving up (if you think the match is OK alter AstromControl.minMatchPerChip)" 
		    << std::endl;
	  throw LSST_EXCEPT(lsst::pex::exceptions::RuntimeError,"ERROR: not enough matches for current chip: giving up (if you think the match is OK alter AstromControl.minMatchPerChip)");
	}
	...
	{
	  unsigned match_order = AstromControl.distortionDegree;
	  // ... if possible
	  while (((match_order+1)*(match_order+2))/2 > subList.size()) 
	    {match_order--;}
	  if (match_order != AstromControl.distortionDegree) 
